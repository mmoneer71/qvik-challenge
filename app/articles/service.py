...
def get_all_articles(db_session: Session) -> List[APIArticle]:
    db_articles = db_session.query(DbArticle).all()
    api_articles = list()
...
def update_article_channel(db_session: Session, article_id :int,  new_channel_name: str) -> APIArticle:
    try:
...
def get_articles_by_word_count(db_session: Session, min_words: int, max_words: int):
    db_articles = db_session.query(DbArticle).filter(DbArticle.word_count >= min_words).filter(DbArticle.word_count <= max_words)
    api_articles = list()
    for db_article in db_articles:
        api_articles.append(APIArticle(id=db_article.id, url=db_article.url, channel_id=db_article.channel_id, word_count=db_article.word_count))
    return api_articles
