...
def test_get_channel_articles():
    db_session = test_session()
    ArticlesService.create_article(db_session=db_session,article_url=sample_urls[0], channel_id=1)
    ArticlesService.create_article(db_session=db_session, article_url=sample_urls[1], channel_id=1)
    articles = ChannelsService.get_channel_articles(db_session=db_session, channel_id=1)
    assert len(articles) == 2
    assert articles == [Article(id=1, url=sample_urls[0], channel_id=1, word_count=473), 
